import { DataRow, KpiMetrics, CampaignTotals, DailyMetrics, ShowsByDay } from './types';
...
export function calcKpiMetrics(rows: DataRow[], showsByDay: ShowsByDay = {}): KpiMetrics {
  ...
  const totalShows = Object.values(showsByDay).reduce((s, n) => s + n, 0);

  return {
    cpm: totalImpressions > 0 ? (totalSpent / totalImpressions) * 1000 : null,
    ...
    costPerShow: safeDiv(totalSpent, totalShows),
    ...
    showRate: totalSchedules > 0 ? (totalShows / totalSchedules) * 100 : null,
    totalSpent,
    totalLeads,
    totalSchedules,
    totalCheckouts,
...
export function calcDailyMetrics(rows: DataRow[], showsByDay: ShowsByDay = {}): DailyMetrics[] {
  const map = new Map<string, DataRow[]>();
  rows.forEach((r) => {
  ...
  return days.map((day) => {
    ...
    const shows = showsByDay[day] || 0;

    return {
      day,
      costPerLead: leads > 0 ? safeDiv(spent, leads) : 0,
      costPerSchedule: sched > 0 ? safeDiv(spent, sched) : 0,
      costPerCheckout: checkout > 0 ? safeDiv(spent, checkout) : 0,
      costPerShow: shows > 0 ? safeDiv(spent, shows) : 0,
      loadRate: clicks > 0 ? (lpv / clicks) * 100 : 0,
      vslToLead: lpv > 0 ? (leads / lpv) * 100 : 0,
      vslToSchedule: lpv > 0 ? (sched / lpv) * 100 : 0,
      vslToCheckout: lpv > 0 ? (checkout / lpv) * 100 : 0,
      leadToSchedule: leads > 0 ? (sched / leads) * 100 : 0,
      scheduleToCheckout: sched > 0 ? (checkout / sched) * 100 : 0,
      showRate: sched > 0 && shows > 0 ? (shows / sched) * 100 : 0,
      amountSpent: spent,
      leads,
      appointmentsScheduled: sched,
      checkoutsInitiated: checkout,
      shows,
      cpc: clicks > 0 ? safeDiv(spent, clicks) : 0,
